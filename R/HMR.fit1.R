#' @importFrom stats lsfit optimize median ls.diag pt qt 
#' @importFrom graphics par plot points lines text legend polygon mtext locator
#' @importFrom grDevices colors jpeg postscript dev.off

#copied from the HMR package (Version 0.3.1) and modified to return kappa and phi
...
.HMR.fit1<-function(tid,konc,A,V,serie,ngrid,LR.always,FollowHMR,JPG,PS,PHMR,npred,xtxt,ytxt,pcttxt,MSE.zero,bracketing.tol,bracketing.maxiter)
{
  ...
  if (n>2)
  {
    ...
    ### Afgr?nser de mulige kappa-v?rdier
    indramme<-function()
    {
  ...
  if (dacode>0)
  {
    ...
    } else
    {
      ...
      {
        # No flux
        f0.est<-0; f0.se<-NA_real_; f0.p<-NA_real_; f0.lo95<-NA_real_; f0.up95<-NA_real_; method<-'No flux'; advarsel<-'None'; phi <- NA_real_; kappa <- NA_real_
      }
    }
  ...
  #if (method != "HMR") {f0.est<-NA_real_ ; f0.se<-NA_real_; f0.p<-NA_real_}
  ### Output
  list(serie=serie,f0=f0.est,f0.se=f0.se,f0.p=f0.p,
  method=method,warning=advarsel, kappa=kappa, phi=phi)
}
